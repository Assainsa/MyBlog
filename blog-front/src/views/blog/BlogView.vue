        ...
        <div class="me-view-card">
          ...
          <div class="me-view-author">
            ...
            <el-button
              ...
              <el-button
                v-if="(this.article.author.id == this.$store.state.id)||this.$store.state.admin==1"
                @click="deleteArticle()"
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      title() {
        return `${this.article.title} - 文章 - TZSBLOG`
      }
    },
    methods: {
      ...
      deleteArticle(){
        ...
        this.$confirm('确定删除文章?', '提示', {
                  ...
                  deleteArticleById(that.$route.params.id).then(data =>{
                    if(data.success){
      ...
      publishComment() {
        ...
        if(this.$store.state.account.length == 0){
          ...
          return;
        }else if (!that.comment.content) {
          that.$message({type: 'error', message: '评论不能为空', showClose: true})
          return;
        }
